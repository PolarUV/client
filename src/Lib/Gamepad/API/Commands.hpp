...
namespace Gamepad {
    ...
    struct Commands {
        ...
        friend std::ostream &operator<<(std::ostream &out, const Commands &commands);
    };

    inline std::ostream &operator<<(std::ostream &out, const Commands &commands) {
        using MoveEnum = Commands::MoveEnum;
        using GripperEnum = Commands::GripperEnum;

        out << "[COMMANDS STRUCT]\n"
            << "Move: " << commands.Move[MoveEnum::Fx] << ", " << commands.Move[MoveEnum::Fy] << ", "
            << commands.Move[MoveEnum::Fz] << ", " << commands.Move[MoveEnum::Mx] << ", "
            << commands.Move[MoveEnum::My]
            << ", " << commands.Move[MoveEnum::Mz] << '\n'

            << "Gripper: " << commands.Gripper[GripperEnum::Gripper1] << ", "
