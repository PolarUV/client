...
#include "Lib/Renderer/API/imgui_impl_glfw.hpp"
...
#define GLFW_HAS_FOCUS_ON_SHOW                                                                                         \
    (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3300)  // 3.3+ GLFW_FOCUS_ON_SHOW
...
#define GLFW_HAS_GAMEPAD_API                                                                                           \
    (GLFW_VERSION_MAJOR * 1000 + GLFW_VERSION_MINOR * 100 >= 3300)  // 3.3+ glfwGetGamepadState() new api
...
void ImGui_ImplGlfw_CursorPosCallback(GLFWwindow* window, double x, double y) {
    ImGui_ImplGlfw_Data* bd = ImGui_ImplGlfw_GetBackendData();
    if (bd->PrevUserCallbackCursorPos != NULL && window == bd->Window) bd->PrevUserCallbackCursorPos(window, x, y);
